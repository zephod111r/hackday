...
// debug arrow to show vectors
function makeDebugArrow(colour) {
	var arrow = new THREE.ArrowHelper( 
			new THREE.Vector3(1,0,0), new THREE.Vector3(0,2,0), 1.0, 
			colour, 0.25, 0.1);
	return arrow;
}


function updateDebugArrow(arrow, pos, dir, scaler) {
	if (dir.length() > 0.001) {
		scaler = typeof scaler !== 'scaler' ? scaler : 1.0;
		...
		var len = dir.length();
		arrow.setLength(len * scaler, 0.25 * scaler*len, 0.1 * scaler*len);
	} else {
		arrow.setLength(0.01);
	}
