(function () {
	...
	// arrow to debug vectors
	var redArrow = makeDebugArrow(0xff0000);
	scene.add(redArrow);
	// arrow to debug vectors
	var blueArrow = makeDebugArrow(0x0000ff);
	scene.add(blueArrow);
	...
	function update() {
		...
		// update position
		var forwardForce = getForwardVector(thingMesh);
		forwardForce.multiplyScalar(controller.forwardBack());
		var sideForce = getSideVector(thingMesh);
		sideForce.multiplyScalar(controller.strafeLeftRight());
		var force = new THREE.Vector3();
		force.addVectors(forwardForce, sideForce);
		...
		var redDir = new THREE.Vector3();
		redDir.copy(force);
		updateDebugArrow(redArrow, thingMesh.position, redDir, 10);
		redArrow.position.add(new THREE.Vector3(0,1.1,0));
		
		var blueDir = new THREE.Vector3();
		blueDir.copy(thingVel);
		updateDebugArrow(blueArrow, thingMesh.position, blueDir, 10);
		blueArrow.position.add(new THREE.Vector3(0,1,0));
